...
#Description:
#    1.implement iaddl:
#          will not cause new hazard, so the only thing to do is change some HCL code.
#       
#    2.implement leave:
#          cause load/use hazard. the approach to cope with it is same with popl
#    3.改变下述指令行为支持伪跳转表
#           push xxx   
#           ret 
#      使得ret能直接靠转发实现，将2个语句waste cycle压缩到1个
#      而不用等待插入后再取出再跳转。
#      具体实现为修改转发逻辑、修改控制逻辑、同时转发修改PC以后要直接bubble抹掉ret语句
#
#/* $begin pipe-all-hcl */
####################################################################
...
int f_predPC = [
    #f_icode == IJXX && f_ifun!=0 && !(D_icode in {IOPL, IIADDL})
       # && !(E_icode in {IOPL, IIADDL}) && !M_Cnd: f_valP;
